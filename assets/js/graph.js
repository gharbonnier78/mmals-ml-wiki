...
const cats=['all','architecture','routing','memory','geometry','dynamics','signal','systems','predictive','evaluation','control','ecology','semantics','continual-learning'];
const colors={architecture:'#82c95d',routing:'#53b9ac',memory:'#d2aa54',geometry:'#6fa8dc',dynamics:'#8ab4f8',signal:'#79c2b0',systems:'#b7a36b',predictive:'#a78bda',evaluation:'#c08ade',control:'#eea843',ecology:'#9fcf65',semantics:'#b6c3b8','continual-learning':'#d86a6a'};
let active='all';
...
const categoryById={mmals:'architecture','biological-mapping':'semantics',host:'architecture','fungal-medium':'architecture',mycorrhiza:'ecology','inferred-context':'routing',route:'routing','functional-route':'memory','functional-memory':'memory','reconstructive-synthetic-memory':'memory','host-specialization':'ecology','probability-simplex':'geometry',torus:'geometry','stability-plasticity':'continual-learning',cal:'evaluation',tput:'control',ecospec:'control','outcome-bearing-execution':'evaluation','minimal-sufficient-dynamic-inference':'continual-learning','stochastic-dynamics':'dynamics','deterministic-chaos':'dynamics','dynamical-regime':'dynamics','observability-reconstruction':'signal','local-to-global-structure':'systems','local-geometry':'geometry','dynamic-compatibility-distance':'routing','world-model':'predictive'};
const ring=concepts.filter(c=>c.id!=='mmals');
const pos={mmals:{x:cx,y:cy}};ring.forEach((c,i)=>{const a=-Math.PI/2+i*2*Math.PI/ring.length;const rr=i%2?235:185;pos[c.id]={x:cx+Math.cos(a)*rr,y:cy+Math.sin(a)*rr}});
...
function draw(){svg.innerHTML='';svg.setAttribute('viewBox',`0 0 ${W} ${H}`);
 ...
 relations.filter(r=>visible.has(r.source)&&visible.has(r.target)).forEach(r=>{const p=pos[r.source],q=pos[r.target];if(!p||!q)return;const line=el('line',{x1:p.x,y1:p.y,x2:q.x,y2:q.y,stroke:'#33433a','stroke-width':'1.4',opacity:'.8'});svg.appendChild(line)});
 concepts.filter(c=>visible.has(c.id)).forEach(c=>{const p=pos[c.id],cat=categoryById[c.id];const g=el('g',{tabindex:'0',role:'link','aria-label':c.title,style:'cursor:pointer'});const rad=c.id==='mmals'?48:30;g.appendChild(el('circle',{cx:p.x,cy:p.y,r:rad,fill:colors[cat]||'#82c95d',opacity:c.id==='mmals'?'.95':'.82',stroke:'#eff5ed','stroke-width':c.id==='mmals'?'2':'1'}));const t=el('text',{x:p.x,y:p.y+4,'text-anchor':'middle',fill:'#071008','font-family':'system-ui','font-size':c.id==='mmals'?'15':'10','font-weight':'700'});const words=c.title.split(' ');if(words.length>2){const split=Math.ceil(words.length/2);const a=el('tspan',{x:p.x,dy:'-4'});a.textContent=words.slice(0,split).join(' ');const b=el('tspan',{x:p.x,dy:'13'});b.textContent=words.slice(split).join(' ');t.append(a,b)}else t.textContent=c.title;g.appendChild(t);g.onclick=()=>location.href=`../concepts/${c.id}/index.html`;g.onkeydown=e=>{if(e.key==='Enter')g.onclick()};svg.appendChild(g)});
}
draw();
})();
