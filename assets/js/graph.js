...
function draw(){svg.innerHTML='';svg.setAttribute('viewBox',`0 0 ${W} ${H}`);
 ...
 relations.filter(r=>visible.has(r.source)&&visible.has(r.target)).forEach(r=>{const p=pos[r.source],q=pos[r.target];if(!p||!q)return;const line=el('line',{x1:p.x,y1:p.y,x2:q.x,y2:q.y,stroke:'#33433a','stroke-width':'1.2',opacity:'.62'});svg.appendChild(line)});
 concepts.filter(c=>visible.has(c.id)).forEach(c=>{const p=pos[c.id],cat=categoryById[c.id];const g=el('g',{tabindex:'0',role:'link','aria-label':c.title,style:'cursor:pointer'});const rad=c.id==='mmals'?46:25;g.appendChild(el('circle',{cx:p.x,cy:p.y,r:rad,fill:colors[cat]||'#82c95d',opacity:c.id==='mmals'?'.95':'.82',stroke:'#eff5ed','stroke-width':c.id==='mmals'?'2':'1'}));const t=el('text',{x:p.x,y:p.y+4,'text-anchor':'middle',fill:'#071008','font-family':'system-ui','font-size':c.id==='mmals'?'15':'8.5','font-weight':'700'});const words=c.title.split(' ');if(words.length>2){const split=Math.ceil(words.length/2);const a=el('tspan',{x:p.x,dy:'-4'});a.textContent=words.slice(0,split).join(' ');const b=el('tspan',{x:p.x,dy:'11'});b.textContent=words.slice(split).join(' ');t.append(a,b)}else t.textContent=c.title;g.appendChild(t);g.onclick=()=>location.href=`../concepts/${c.id}/index.html`;g.onkeydown=e=>{if(e.key==='Enter')g.onclick()};svg.appendChild(g)});
}
draw();
})();
